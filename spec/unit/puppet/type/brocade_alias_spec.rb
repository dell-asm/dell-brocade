...
describe Puppet::Type.type(:brocade_alias) do
  ...
  describe "when validating values" do
    describe "validating alias_name param" do
      it "should allow a valid alias name where ensure is present" do
        described_class.new(:alias_name => 'alias_demo', :ensure => 'present',:member => '0f:0a:0a:0a:0a:0a:0a:0a')[:alias_name].should == 'alias_demo'
      end
	  it "should allow a valid alias name where ensure is absent" do
        described_class.new(:alias_name => 'alias_demo_new', :ensure => 'absent',:member => '0f:0a:0a:0a:0a:0a:0a:0a')[:alias_name].should == 'alias_demo_new'
      end
